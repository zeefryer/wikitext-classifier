...
def compute_metrics(state, logits, labels, weights=None):
    ...
    loss, denom = compute_weighted_cross_entropy(logits, labels, weights)
    ...
    metrics["acc"] = acc
    metrics["w_acc"] = w_acc
    metrics["batch_size"] = jnp.array(len(labels))
    metrics["denom"] = denom
...
def train_step(state, batch):
    ...
    weights = jnp.where(
        batch["label"] > 0, state.pos_weight, state.neg_weight
    ).astype(jnp.float32)
    ...
    def loss_fn(params):
        ...
        loss, denom = compute_weighted_cross_entropy(
...
def eval_step(state, batch):
    ...
    logits = state.apply_fn(
        {"params": state.params}, batch["inputs"], train=False
    )
    metrics = compute_metrics(state, logits, batch["label"], weights)
...
def main(args):
    ...
    with open(
        train_config["metrics_dir"].parent.joinpath("train_config.yaml"), "w"
    ) as f:
